#!/usr/bin/env python3
"""
El Viaje Profesional - Video Generator (12 minutos máximo)
Genera un video noticiero basado en el script de EduConecta Aburrá Sur
OPTIMIZADO PARA 12 MINUTOS
"""
...
def generate_intro(config, estaciones_data, width, height):
    """Genera la introducción - OPTIMIZADA A 1.5 MINUTOS"""
    clips = []
    ...
    # Presentación Ana (20 segundos - resumida)
    bg_clip2 = create_color_clip(width, height, config['colors']['background'], 20)
    speaker_text = create_text_clip(
        f"ANA SOFÍA\n\nBienvenidos a TdeA Noticias.\nVamos a acompañar a dos estudiantes en su viaje profesional.",
        20, fontsize=36, color='white', width=width
    )
    ana_clip = CompositeVideoClip([bg_clip2, speaker_text], size=(width, height))
    clips.append(ana_clip)
    
    # Presentación Juan Diego (20 segundos - resumida)
    bg_clip3 = create_color_clip(width, height, config['colors']['background'], 20)
    speaker_text2 = create_text_clip(
        f"JUAN DIEGO\n\nTenemos 4 estaciones para encontrar una solución tecnológica para la brecha digital.",
        20, fontsize=36, color='white', width=width
    )
...
def generate_estaciones(config, estaciones_data, width, height):
    """Genera las 4 estaciones - OPTIMIZADAS A 8 MINUTOS (2 MIN CADA UNA)"""
    all_clips = []
    
    for estacion in estaciones_data['estaciones']:
        estacion_clips = []
        
        # Título de la estación (15 segundos)
        bg_title = create_color_clip(width, height, config['colors']['background'], 15)
        title_text = create_text_clip(
            f"ESTACIÓN {estacion['numero']}\n{estacion['titulo']}",
            15, fontsize=56, color=estacion['color'], width=width
        )
        title_clip = CompositeVideoClip([bg_title, title_text], size=(width, height))
        estacion_clips.append(title_clip)
        
        # Pregunta (10 segundos)
        bg_q = create_color_clip(width, height, config['colors']['background'], 10)
        q_text = create_text_clip(
            f"Pregunta:\n{estacion['pregunta']}",
            10, fontsize=32, color='#fbbf24', width=width
        )
        q_clip = CompositeVideoClip([bg_q, q_text], size=(width, height))
        estacion_clips.append(q_clip)
        
        # Contenido resumido (75 segundos)
        bg_content = create_color_clip(width, height, config['colors']['background'], 75)
        
        # Crear resumen conciso
        if 'key_points' in estacion:
            content_text = "Puntos Clave:\n" + "\n".join([f"• {p}" for p in estacion['key_points'][:2]])
        elif 'competencias' in estacion:
            content_text = "Competencias Clave:\n" + "\n".join([f"• {c}" for c in estacion['competencias'][:3]])
        elif 'grupos_investigacion' in estacion:
            content_text = "Rutas Académicas:\n" + "\n".join([f"• {g}" for g in estacion['grupos_investigacion']])
        else:
            content_text = "Responsabilidades:\n" + "\n".join([f"• {r}" for r in estacion['responsabilidades'][:3]])
...
def generate_solution(config, estaciones_data, width, height):
    """Genera la sección de solución - OPTIMIZADA A 1.5 MINUTOS"""
    clips = []
    
    solucion = estaciones_data['solucion_final']
    
    # Título (10 segundos)
    bg_clip = create_color_clip(width, height, config['colors']['background'], 10)
    title_text = create_text_clip(
        f"{solucion['nombre']}",
        10, fontsize=56, color='#fbbf24', width=width
    )
    title_clip = CompositeVideoClip([bg_clip, title_text], size=(width, height))
    clips.append(title_clip)
    
    # Descripción y características (80 segundos)
    bg_desc = create_color_clip(width, height, config['colors']['background'], 80)
    desc_text = create_text_clip(
        solucion['descripcion'] + "\n\n" + 
        "\n".join([f"✓ {c}" for c in solucion['caracteristicas'][:2]]),
        80, fontsize=32, color='white', width=width
    )
...
def generate_closing(config, estaciones_data, width, height):
    """Genera el cierre - OPTIMIZADO A 1.5 MINUTOS"""
    clips = []
    ...
    text = create_text_clip(
        cierre['mensaje_final'] + "\n\n¡Gracias por acompañarnos\nen nuestro viaje profesional!",
        30, fontsize=44, color='#fbbf24', width=width
    )
...
def main():
    """Función principal."""
    print("🎬 El Viaje Profesional - Video Generator (12 MINUTOS)")
    print("=" * 60)
    ...
    print(f"\n⏱️  Duración total: {duracion_total:.0f} segundos ({duracion_total/60:.1f} minutos)")
    
    if duracion_total > 720:  # 12 minutos = 720 segundos
        print("⚠️  ADVERTENCIA: El video excede 12 minutos. Ajustando...")
    
    # Guardar video
    output_path = OUTPUT_DIR / 'el_viaje_profesional_12min.mp4'
    ...
    print(f"⏱️  Duración final: {video_final.duration:.0f} segundos ({video_final.duration/60:.1f} minutos)")
